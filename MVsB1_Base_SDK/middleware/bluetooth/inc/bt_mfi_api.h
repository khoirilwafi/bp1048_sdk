...
typedef enum{
	 ...
	 * the union params.bd_addr is valid
	 */
	 ...
	 * the union params.mfiReceivedData is valid and paramsLen indicate
	 ...
	 * the union params.mfiSentData is valid and paramsLen indicate
	 * the length of data sent.
	 *
	 * @note
	 * The memory allocated by the data can be release here.
	 */
...
void BtMfiCallback(BT_MFI_CALLBACK_EVENT event, BT_MFI_CALLBACK_PARAMS * param);
 ...
 *			BT_STACK_EVENT_MFI_DATA_SENT received. So the variable
...
/**
 * @brief  send data via mfi channel is ready?
 * @param  NONE
 * @return 1=ready, 0=not ready
 * @Note
 *		
 */
bool MFiIsReady(void);

/**
 * @brief  MFi profile initial
 * @param  NONE
 * @return 1=success
 * @Note
 *		
 */
bool MFiAppInit(BTMfiCallbackFunc callback);
